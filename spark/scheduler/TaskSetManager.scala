...
//marvin
  lazy val blockDestinationScheduler =  sched.backend.initBlockDestinationScheduler(this)
  /*
  ...
  //marvin  记录要正在被push的task


  def handleTashPushFinished(tid:Long,result: DirectTaskResult[_],
                             mapStatusGroup:Array[MapStatus]) = {
    val info = taskInfos(tid)
    val index = info.index
    val task = tasks(index)
    sched.dagScheduler.taskEnded(
      tasks(index), Success, mapStatusGroup, result.accumUpdates, info, result.metrics)
  }
  ...
  def handleSuccessfulTask(tid: Long, result: DirectTaskResult[_]) = {
    ...
    task match{
      case smt:ShuffleMapTask =>
        val outputValue = result.value()
        val mapStatusWrapper = outputValue.asInstanceOf[MapStatusWrapper]
        blockDestinationScheduler.handleTaskPush(tid,task.partitionId,result,mapStatusWrapper)

       /* sched.dagScheduler.taskEnded(
          tasks(index), Success, mapStatusWrapper.mapStatus, result.accumUpdates, info, result.metrics)*/
