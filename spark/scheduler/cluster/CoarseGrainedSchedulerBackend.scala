...
import org.apache.spark.scheduler.cluster.BlockDestinationScheduler
  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  //marvin
  val blockManagerIdByExecutor = new mutable.HashMap[String, BlockManagerId]()
  ...
  class DriverActor(sparkProperties: Seq[(String, String)]) extends Actor with ActorLogReceive {
    ...
    def receiveWithLogging = {
      ...
      //marvin
      case PushFinished(tid,executorId) =>
        logInfo("%%%%%% tid " + tid + " executorId " + executorId + " pushFinished %%%%%%")
        tidToBlockDestinationScheduler(tid).handleTaskPushResult(tid,executorId)

      case RegisterBlockManagerId(serBlockManagerId) =>
        val ser = SparkEnv.get.closureSerializer.newInstance()
        val blockManagerId = ser.deserialize[BlockManagerId](serBlockManagerId.value)
        blockManagerIdByExecutor(blockManagerId.executorId) = blockManagerId
        logInfo("%%%%%% registerBlockManager id is " + blockManagerId + " %%%%%%")
    }
  ...
  //marvin
  def broadResult(executorIdToShuffleBlockInfo: HashMap[String, ShuffleBlockInfo]){
    val ser = SparkEnv.get.closureSerializer.newInstance()
    executorIdToShuffleBlockInfo.foreach { case (executorId, shuffleBlockInfo) =>
      executorDataMap(executorId).executorActor! PushBlock(
        new SerializableBuffer(ser.serialize[ShuffleBlockInfo](shuffleBlockInfo)))
    }
  }

  val tidToBlockDestinationScheduler = new HashMap[Long,BlockDestinationScheduler]

  override def initBlockDestinationScheduler(taskSetManager: TaskSetManager)
  : BlockDestinationScheduler ={
    val blockDestinationScheduler = new BlockDestinationScheduler(
      this,taskSetManager, executorDataMap,blockManagerIdByExecutor)
    blockDestinationScheduler
  }

  def registerTidToBlockDestinationScheduler(tid:Long,
                                             blockDestinationScheduler: BlockDestinationScheduler): Unit ={
    tidToBlockDestinationScheduler.put(tid,blockDestinationScheduler)
  }
